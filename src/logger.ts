...
class Logger {
	...
	logRequestAndResponse(request: any, response: any, time: number) {
		...
		this.info(
			...
			isError ? ERROR_COLOR(String(statusCode)) : SUCCESS_COLOR(String(statusCode)),
			dim(`${time}ms`),
			...
			...(this.verbose
				? [
						REQUEST_TYPE_COLOR("\nRequest body: "),
						formatBody(reqBody),
						...(!isError ? [SUCCESS_COLOR("\nResponse body: "), formatBody(resBody)] : []),
					]
				: []),
		)
	}

	logMessage(action: Actions, message: any) {
		const { type, payload } = message
		const symbol = action === "sent" ? SENT_SYMBOL : RECEIVED_SYMBOL
		const isError = isErrorMessage(type)
		const primaryColor = isError ? ERROR_COLOR : action === "received" ? REQUEST_TYPE_COLOR : SUCCESS_COLOR
		const messageBody = isError ? formatErrorPayload(payload) : formatMessagePayload(action, message, this.verbose)
...
function formatResponseError({ name, message, status }: any) {
	if (name === undefined && message === undefined && status === undefined) return ""
...
function formatMessagePayload(action: Actions, message: any, verbose: boolean) {
	const { type, payload } = message
...
function formatTriplesPayload(action: Actions, payload: any, verbose: boolean) {
	if (verbose) return JSON.stringify(payload, null, 2)
	const { triples } = payload
	return dim(`${action} ${triples.length} triple(s)`)
}

function formatConnectQueryPayload(payload: any, verbose: boolean) {
	if (payload === undefined || !verbose) return ""
	if (verbose) {
		return JSON.stringify(payload, null, 2)
	}
